...
{
	timer_4.stop_timer(); // Выключаем таймер;
	timer_4.clearTimerInterruptFlag(TIMER_OVERFLOW_INT); // запрещаем все прерывания от этого таймера.
...
int main() {
	...
	timer_3.clear();
	timer_3.setTimerInterruptFlag(TIMER_OVERFLOW_INT);
	timer_3.setCNT(-8000);
	timer_3.start_timer(CLK_DIV_1);
	...
	timer_3.clearTimerInterruptFlag(TIMER_OVERFLOW_INT); // Запрещаем прерывание при переполнении таймера 3.
	...
	while (1) {
		while (key_mode)		// В режиме набора текст:
		{
			...
			timer_4.setCNT(0); // Дополнительное время работы в этом режиме
			...
			LEDS |= 0x0F&key1;
			putchar(key1);
			timer_3.setTimerInterruptFlag(TIMER_OVERFLOW_INT); // пик
			for(i=0; i<20; i++) _delay_ms(10);
			timer_3.clearTimerInterruptFlag(TIMER_OVERFLOW_INT);
		}
		...
		while(sec == time.Second)
		{
			...
			if (key1 != '\n')
			{
				...
				timer_4.start_timer(CLK_DIV_256); // Предделитель: F_CPU/256
				timer_4.setTimerInterruptFlag(TIMER_OVERFLOW_INT); // Разрешаем прерывание по переполнению
				timer_4.setCNT(0);
